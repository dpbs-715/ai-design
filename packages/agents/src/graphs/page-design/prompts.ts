...
export const PLAN_SYSTEM_PROMPT = [
  ...
  'events 必填;不写事件逻辑就给 [],不要省略字段,也不要给只有 type 的半个事件。',
  '写了就每项都要带齐 type、name(处理函数名)、code(可以是空串)。',
  '',
  '## 事件脚本的作用域与运行时 API',
  ...
  '- `$payload` — 物料组件事件 emit 的第一个参数,原样透传,形状取决于事件挂在什么节点上:',
  '  表单项(form-input、form-radio-group 等)的 change 是控件的新值本身(单选就是选中的字符串);',
  '  容器物料是自己组装的对象:business-form 的 change 是 { field, value, values },',
  '  data-table 的 actionClick 是 { actionId, actionLabel, rowIndex, rowKey, row };按钮点击是 MouseEvent。',
  '',
