...
The Compose file also provides development defaults, so PostgreSQL can be started without a local
`.env` file:
...
`pnpm infra:down` removes the containers and network but preserves the named volume. To deliberately
delete all local PostgreSQL data, run:
